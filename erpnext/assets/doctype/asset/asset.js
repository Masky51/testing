		...
		if (frm.doc.docstatus==1) {
			if (in_list(["Submitted", "Partially Depreciated", "Fully Depreciated"], frm.doc.status)) {
				...
				frm.add_custom_button(__("Generate QR Code"), function() {
					erpnext.asset.generate_qrcode(frm);
				}, __("Manage"));
...
erpnext.asset.generate_qrcode = function(frm) {
	//frappe call
	frappe.call({
		args: {
			"assetname": frm.doc.name
		},
		method: "erpnext.assets.doctype.asset.asset.make_generate_qrcode",
		callback: function (r) {
			cur_frm.reload_doc();
			var assetName = frm.doc.name;
			frappe.show_alert("QR code generated for " + assetName, 5, "success");
		}
	});
};
