...
@frappe.whitelist()
def concatenateStrings(api, assetname):

		constring = api + assetname
		return constring
...
@frappe.whitelist()
def make_generate_qrcode(assetname):

	api = "http://erpnext_new.com:8003/app/asset/"
	# Call the function to concatenate two strings
	result = concatenateStrings(api, assetname)
	generate_qrcode = frappe.get_doc(
		{
			"doctype": "QR Code Generator",
			"long_url": result,
			"short_url": assetname,
			"asset_name": assetname
		}
	).insert()
	generate_qrcode.submit()
	frappe.msgprint("QR code generated for "+ assetname, indicator="green", title="Success")

#method 3
@frappe.whitelist()
def generate_qrcode(assets):
    import json
    assets = json.loads(assets)
    ...
    for asset in assets:
        ...
        itemcode = asset.get('asset_name')
        location = asset.get('location')
        print("\n\r\n\r\n\r\n\r" +  f"{assetname}" + "\n\r\n\r\n\r\n\r")
        print("\n\r\n\r\n\r\n\r" +  f"{itemcode}" + "\n\r\n\r\n\r\n\r")
        print("\n\r\n\r\n\r\n\r" +  f"{location}" + "\n\r\n\r\n\r\n\r")

        asset_doc = frappe.get_doc("Asset", assetname)

        if not asset_doc:
            frappe.throw(f"Asset '{assetname}' not found.")

		# Check if QR code already exists for the asset
        existing_qr_code = frappe.get_all("QR Code Generator", filters={"name": assetname})

        if existing_qr_code:
            frappe.msgprint(f"QR code already exists for Asset" + f"{assetname}")
            continue  # Skip generating QR code for this asset
    ...
    frappe.msgprint("QR code generated for assets", indicator="green", title="Success")